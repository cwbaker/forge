...
/**
// Set whether or not this Target always binds to the filesystem.
//
// Setting always bind to true allows a directory target to be used to trigger
// other targets or the dependency graph itself to be outdated when directory
// contents are changed.  This is useful when files or the dependency graph 
// are generated by scanning the filesystem.  See Target::bind_to_file() for
// a fuller description of how binding affects the outdated flag and 
// timestamps of targets in the dependency graph.
//
// @param always_bind
//  True to force this Target to always bind to the filesystem or false to 
//  ignore binding for directories that have no dependencies.
*/
void Target::set_always_bind( bool always_bind )
{
    always_bind_ = always_bind;
}

/**
// Is this Target always bound to the filesystem?
//
// @return
//  True if this Target is always bound to the filesystem otherwise false.
*/
bool Target::always_bind() const
{
    return always_bind_;
}
...
//
// The index is resolved into both the explicit and implicit dependencies of
// this Target by treating all of the dependencies to be in a single array 
// with the explicit dependencies listed first.  A value of 'n' in the range
// [0, #explicit) selects an explicit dependency, a value of 'n'
// in the range [#explicit, #explicit + #implicit] selects an implicit 
// dependency, any of value of 'n' selects null.
//
// @param n
