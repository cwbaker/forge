...
std::filesystem::path absolute( const std::filesystem::path& path, const std::filesystem::path& base_path )
{
    ...
    std::filesystem::path absolute_path( base_path );
    absolute_path /= path;
    absolute_path = absolute_path.lexically_normal();

    // Remove trailing '.' elements that are left by lexically_normal() but
    // that aren't what the build tool expects in an absolute path.
