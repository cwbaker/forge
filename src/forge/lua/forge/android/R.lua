...
function R.build( toolset, target )
    ...
    local command_line = {
        ...
        '-f',
        '-m',
        ('-I "%s"'):format( android.android_jar(toolset) ),
    ...
    local aapt = ('%s/aapt'):format( toolset.settings.android.build_tools_directory );
    system( aapt, command_line, nil, toolset:filenames_filter(target) );
end
