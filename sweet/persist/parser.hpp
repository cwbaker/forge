...
#ifndef SWEET_PERSIST_PARSER_HPP_INCLUDED
#define SWEET_PERSIST_PARSER_HPP_INCLUDED
...
// A lexical analyzer.
*/
template <class Iterator, class Char = typename std::iterator_traits<Iterator>::value_type, class Traits = std::char_traits<Char>, class Allocator = std::allocator<Char> >
class Lexer
{
...
template <class Iterator, class UserData, class Char = typename std::iterator_traits<Iterator>::value_type, class Traits = std::char_traits<Char>, class Allocator = std::allocator<Char> >
class ParserEventSink
{
    public:
        virtual ~ParserEventSink();
        virtual void parser_error( const Parser<Iterator, UserData, Char, Traits, Allocator>* parser, const char* message );
...
class AddParserActionHandler
{
    ...
    typedef ParserEventSink<Iterator, UserData, Char, Traits, Allocator> ParserEventSink;
    typedef UserData (*ParserActionFunction)( int symbol, const ParserNode* start, const ParserNode* finish, ParserEventSink* context );
...
class Parser
{
    public:
        typedef ParserNode<UserData, Char, Traits, Allocator> ParserNode;
        typedef typename std::vector<ParserNode>::const_iterator ParserNodeConstIterator_;
        typedef ParserEventSink<Iterator, UserData, Char, Traits, Allocator> ParserEventSink;
        typedef void (*LexerActionFunction)( Iterator* begin, Iterator end, std::basic_string<Char, Traits, Allocator>* lexeme, int* symbol );
        typedef UserData (*ParserActionFunction)( int symbol, const ParserNode* start, const ParserNode* finish, ParserEventSink* context );
    ...
    public:
        Parser( const ParserStateMachine* state_machine, ParserEventSink* event_sink = NULL );
...
template <class Iterator, class UserData, class Char, class Traits, class Allocator>
void 
ParserEventSink<Iterator, UserData, Char, Traits, Allocator>::parser_error( const Parser<Iterator, UserData, Char, Traits, Allocator>* parser, const char* message )
{
}
...
template <class Iterator, class UserData, class Char, class Traits, class Allocator>
Parser<Iterator, UserData, Char, Traits, Allocator>::Parser( const ParserStateMachine* state_machine, ParserEventSink* event_sink )
: state_machine_( state_machine ),
...
Parser<Iterator, UserData, Char, Traits, Allocator>::fire_error( const char* format, ... ) const
{
    ...
    if ( event_sink_ )
    {
        event_sink_->parser_error( this, message );
    }
    else
    {
...
Parser<Iterator, UserData, Char, Traits, Allocator>::debug_reduce( const ParserProduction* reduced_production, const ParserNode* start, const ParserNode* finish ) const
{
    ...
    if ( debug_enabled_ )
    {
        printf( "REDUCE: %d <- ", reduced_production->symbol_ );        
        ...
        if ( node != finish )
        {
            int symbol = node->symbol_;
            const std::string& lexeme = node->lexeme_;
            printf( "(%d %s)", symbol, lexeme.c_str() );
            ++node;
        }
...
Parser<Iterator, UserData, Char, Traits, Allocator>::handle( const ParserProduction* reduced_production, const ParserNode* start, const ParserNode* finish ) const
{
    ...
    return default_action_handler_ != NULL ? (*default_action_handler_)( reduced_production->symbol_, start, finish, event_sink_ ) : UserData();
}
