...
/**
// Set the BindType of this Target.
//
// @param bind_type
//  The BindType to set this Target to have.
*/
void Target::set_bind_type( int bind_type )
{
    SWEET_ASSERT( bind_type >= BIND_NULL && bind_type < BIND_TYPE_COUNT );
    if ( bind_type < BIND_NULL || bind_type >= BIND_TYPE_COUNT )
    {
...
/**
// Get the BindType of this Target.
//
// @return
//  The BindType of this Target.
*/
...
{    
    switch ( get_bind_type() )
    {
        ...
        case BIND_SOURCE_FILE:
            bind_to_file();
            bind_as_source_file();
            break;

        case BIND_INTERMEDIATE_FILE:
            bind_to_file();
            bind_as_intermediate_file();
            break;
            
        case BIND_GENERATED_FILE:
            bind_to_file();
            bind_as_generated_file();
            break;
...
/**
// Bind this Target as a phony.
//
// Sets the timestamp of this Target to be the latest of any of its 
// dependencies and this Target to be outdated if any of its dependencies
// are.
*/
void Target::bind_as_phony()
{
...
/**
// Bind this Target to a source file.
//
...
*/
void Target::bind_as_source_file()
{
    if ( !bound_to_dependencies_ )
    {
        SWEET_ASSERT( bound_to_file_ );    
...
/**
// Bind this Target to an intermediate file.
//
// Sets the timestamp of this Target to be the latest of the last write
// time of the file that it is bound to or the latest timestamp of any of 
// its dependencies and sets this Target to be outdated if any of its 
// dependencies have a later timestamp or are outdated.
*/
void Target::bind_as_intermediate_file()
{
    if ( !bound_to_dependencies_ )
    {
        SWEET_ASSERT( bound_to_file_ );    
        ...
        for ( vector<Target*>::const_iterator i = dependencies_.begin(); i != dependencies_.end(); ++i )
        {
            Target* dependency = *i;
            SWEET_ASSERT( dependency );
            outdated = outdated || dependency->is_outdated() || dependency->get_timestamp() > timestamp;
            timestamp = std::max( timestamp, dependency->get_timestamp() );
        }
...
//
// Sets the timestamp of this Target to be the last write time of the file
// that it is bound to and sets this Target to be outdated if any of its 
// dependencies have a later timestamp or are outdated.
*/
void Target::bind_as_generated_file()
{
    if ( !bound_to_dependencies_ )
    {
        SWEET_ASSERT( bound_to_file_ );    
        ...
        if ( timestamp != 0 )
        {
            ...
            while ( i != dependencies_.end() && !(*i)->is_outdated() && (*i)->get_timestamp() <= timestamp )
            {
                ++i;
            }

            set_timestamp( timestamp );
            set_outdated( i != dependencies_.end() );    
        }
